...
int main()
{
    ...
    RelativeExpiration expiration1(30*60); // 30m relative expiration
    ASSERT("relative expiration 1: 30m", bytes_to_hex(expiration1.encode()), "49 0F");

    RelativeExpiration expiration2(23*60*60); // 23h relative expiration
    ASSERT("relative expiration 2: 23h", bytes_to_hex(expiration2.encode()), "49 6E");

    RelativeExpiration expiration3(4*24*60*60); // 4d relative expiration
    ASSERT("relative expiration 3: 4d", bytes_to_hex(expiration3.encode()), "49 B0");

    RelativeExpiration expiration4(32*24*60*60); // 32d relative expiration
